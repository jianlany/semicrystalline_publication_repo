...
//! Negative values for end mean start from the back of the string.
inline std::string join(const std::vector<std::string> &v, int pos=0, int len=~0)
{           
    std::string s = "";
    auto begin_iter = v.begin() + pos;
    auto end_iter   = v.end();
    // len was default value, copy until end of string. 
    if (len == ~0)    end_iter = v.end();
    // len was negative, count backwards like python.
    else if (len < 0) end_iter = v.end() + len;
    // len is positive, count forwards from current position.
    else if (len > 0) end_iter = begin_iter + len;

    for (auto iter=begin_iter; iter!=end_iter; ++iter) {
        if (iter != begin_iter) s += " ";
        s += *iter;
    }
    return s;
}
